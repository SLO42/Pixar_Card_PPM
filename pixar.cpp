...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/12/07 00:55:35 by saolivei          #+#    #+#             */
/*   Updated: 2018/12/07 00:55:35 by saolivei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
F	S(V p,I&m)
{
	...
	V	f=p;
	f.z=0;
	char l[]="505+5W9W5_9_COC_AOEOA_E_IOQ_I_QOUOY_Y_]OWW[WaOa_aWeWa_e_cWiO";
	for(I i =0;i<60;i+=4)
	{
		V b=V(l[i]-79,l[i+1]-79)*.5,e=V(l[i+2]-79,l[i+3]-79)*.5+b*-1,o=f+(b+e*L(-L((b+f*-1)%e/(e%e),0),1))*-1;
		d=L(d,o%o);
	}
	d=sqrtf(d);V a[]={V(-11,6),V(11,6)};
	for(I i=2;i--;)
	{
		V o=f+a[i]*-1;
	...
	m=1;
	F r=L(-L(B(p,V(-30,-.5,-30),V(30,18,30)),B(p,V(-25,17,-25),V(25,20,25))),B(V(fmodf(fabsf(p.x),8),p.y,p.z),V(1.5,18.5,-25),V(6.5,20,25)));
	if(r<d)
		d=r,m=2;
	F s=19.9-p.y;
	if(s<d)
...
{
	I w=960,h=540,s=16;
	V e(-22,5,25),g=!(V(-3,4,0)+e*-1),l=!V(g.z,0,-g.x)*(1./w),u(g.y*l.z-g.z*l.y,g.z*l.x-g.x*l.z,g.x*l.y-g.y*l.x);
